...
int countingSemArraySHMID, pqSHMID, binSemID, bufferIndexSHMID, numProdSHMID, prodSumSHMID, 
...
int *bufferIndex, *numProd, *prodSum, *pidSize, *pids;
//variables to determine execution and waiting time
...
//insert function that uses bufferIndex to place the given job into the queue
void insertbuffer(pj job) {
    //cant place jobs out of range
    if ((*bufferIndex) < 30) {
        //need to dereference bufferIndex before incrementing, otherwise the pointer address gets increased rather than the value
        printQueue[(*bufferIndex)++] = job;
    } 
    else {
        printf("Buffer overflow\n");
    }
}
//dequeue function that returns the job at the current bufferIndex, else returns an empty job
pj dequeuebuffer() {
    pj job;
    job.numBytes = 0;
    if ((*bufferIndex) > 0) {
        //once again, must dereference before modifying the value of bufferIndex
        return printQueue[--(*bufferIndex)]; // buffer_index-- would be error!
    } else {
...
void *consumer(void *thread_n) {
    ...
    //infinite loop
    while (1) {
        //sleeps to let producers add to queue first
        usleep(100000);
        //if the queue is empty, then calls termHandler to terminate the program (NORMAL termination)
...
void detachMemory(){
    ...
    shmctl(bufferIndexSHMID, IPC_RMID, NULL);
...
void termHandler(){
    if(getpid() == ppid){
        ...
        printf("Number of Jobs Consumed: %d\nNumber of Bytes Consumed: %d\n", numCons, conSum);
...
int main(int argc, char *argv[]) {
    ...
    if(num_of_consumers == 0 || num_of_producers == 0){
        exit(0);
    }
    ...
    //put the bufferIndex into shared memory
    bufferIndexSHMID = shmget(IPC_PRIVATE, sizeof(int), IPC_CREAT | 0666);
    bufferIndex = (int *) shmat(bufferIndexSHMID, NULL, 0);
